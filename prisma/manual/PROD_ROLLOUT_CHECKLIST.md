...
| 9 | `wp_quest_referral_usewp_wave6.sql` | **Revamp: Referral % + Quest tiers + Use WP 3-models + security.** `app_users.referralRateBps` (int, default 1000 = 10%) + `referralRateUpdatedBy`/`At` audit (Finding 7); `wp_ledger(appUserId,refType,refId)` index (referral idempotency); `redemptions.appUserId` (FK→app_users, SetNull) +index + **backfill** (only emails mapping to exactly ONE account — Finding 2 sybil fix); `wp_redemptions` shipping (recipientName/Phone/shippingAddress) + crypto (walletAddress/payoutTxHash); `wp_rewards.cryptoAsset/cryptoAmount/expiresAt` (+ `CRYPTO` category value, TEXT); `quests.milestoneBaseWp/milestoneLadder`. Idempotent, additive; the only write is the safe redemption backfill. **NOTE: flat referral bonus is now DISABLED in code (%-only); `wpReferrerBonusWp`/`wpRefereeWelcomeWp` columns kept but unused.** Ships with the whole revamp (referral %, tiered claims, 3-model Use WP, account-scoped deposit gate). | ⬜ pending — apply before deploying wave6 code |
