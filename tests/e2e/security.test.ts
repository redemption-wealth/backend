...
describe("Security Hardening", () => {
  ...
  test("admin cannot access owner-only routes", async () => {
    ...
    // Only check genuinely owner-only routes. NOTE: GET /api/admin/settings is
    // deliberately admin-readable since #27 (voucher price preview) — writes
    // stay manager-only and are asserted below.
    const routes = [
      "/api/admin/admins",
    ];
